...
function Header() {
    return (
        <h1 className="text-2xl font-semibold text-white p-2">Get Things Done!!👍🏻</h1>
    )
}
...
function LowPriority() {
    return (
        <div className="flex justify-between items-center mx-10 text-sm ">
        <span className="font-semibold text-red-800">Priority</span>
        <span>Low⬇️</span>
    </div>
    )
            ...
            {filteredTasks.map((task, index) => (
                <li key={task.id} draggable onDragStart={(e) => handleDragStart(e, index)} onDrop={(e) => handleDrop(e, index)} className="flex justify-between border-b pt-2 pb-2">
                    <div className="flex gap-2">
                        ...
                        {editTask.id === task.id ? (
                            <>
                                <input type="text" value={editTask.name} onChange={handleEditChange} />
                    ...
                    <div className="flex gap-5">
                        <button type="button" onClick={() => handleEditClick(task.id, task.name)}>✏️</button>
                        <button
                            type="button"
